...
assert(toolbar.includes('onClick={() => onRun()}'), 'run icon must invoke the shared query execution callback without forwarding MouseEvent')
assert(toolbar.includes('onClick={() => onExplain()}'), 'query plan icon must invoke the shared explain callback without forwarding MouseEvent')
assert(toolbar.includes('onClick={() => onFormat()}'), 'format icon must invoke the shared format callback without forwarding MouseEvent')
assert(!toolbar.includes('DropdownMenu'), 'run, query plan, and format must not be hidden in an overflow menu')
...
assert(iconButton.includes('TooltipTrigger\n        render='), 'icon actions must use the actual button as the tooltip trigger')
assert(!iconButton.includes('render={<span'), 'icon actions must not wrap mouse targets in a tooltip span')
