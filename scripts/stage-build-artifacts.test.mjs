...
test('stages fixed-name Windows artifacts and replaces stale output', async (context) => {
  const paths = await fixture('windows', {
    'msi/VaporLensDB_0.9.0_x64_en-US.msi': 'msi-package',
    'nsis/VaporLensDB_0.9.0_x64-setup.exe': 'nsis-package',
  })
...
test('stages AppImage, DEB, and RPM using fixed Linux names', async (context) => {
  const paths = await fixture('linux', {
    'appimage/vapor-lens-db_0.9.0_amd64.AppImage': 'appimage-package',
    'deb/vapor-lens-db_0.9.0_amd64.deb': 'deb-package',
    'rpm/VaporLensDB-0.9.0-1.x86_64.rpm': 'rpm-package',
  })
  context.after(() => rm(paths.root, { recursive: true, force: true }))
  await chmod(join(paths.bundleDir, 'appimage/vapor-lens-db_0.9.0_amd64.AppImage'), 0o755)
