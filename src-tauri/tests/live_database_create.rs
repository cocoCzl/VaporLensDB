...
use vapor_lens_db_lib::{
    drivers::{mysql::MysqlDriver, postgres::PostgresDriver, trait_def::DatabaseDriver},
};
...
async fn mysql_create_database_is_visible_and_duplicate_is_rejected() {
    ...
    let create = format!("CREATE DATABASE `{name}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");

    driver.execute_query(&create, Some("create")).await.expect("create database");
    let visible = driver.get_databases().await.expect("list databases").iter().any(|item| item.name == name);
    let duplicate_rejected = driver.execute_query(&create, Some("duplicate")).await.is_err();
    let cleanup = driver.execute_query(&format!("DROP DATABASE IF EXISTS `{name}`"), Some("cleanup")).await;
...
async fn postgres_create_database_is_visible_and_duplicate_is_rejected() {
    ...
    let driver = PostgresDriver::connect(&url).await.expect("connect to PostgreSQL");
    let name = unique_name("vaporlensdb_smoke");
